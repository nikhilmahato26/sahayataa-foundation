...
export default function Footer() {
  ...
  return (
    <footer className="mt-20">
      {/* Main footer */}
      <div className="bg-brand-400 text-ink">
        <div className="container-x grid gap-10 py-14 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <p className="mt-4 max-w-xs text-sm text-ink-soft">{org.mission}</p>
            <div className="mt-5 flex gap-2">
              {socials.map((s) => (
                <a
                  ...
                  href={s.href}
                  aria-label={s.label}
                  className="grid h-10 w-10 place-items-center rounded-xl bg-ink/5 transition hover:bg-ink/10 text-ink"
                >
          ...
          {/* Contact */}
          <div>
            <h4 className="text-sm font-bold uppercase tracking-wider text-brand-900">
              Reach us
            </h4>
            <ul className="mt-4 space-y-3 text-sm">
              <li>
                <a href={telLink()} className="flex items-center gap-2 hover:underline">
                  <Icon name="Phone" className="h-4 w-4 text-brand-800" />
                  {org.phone}
                </a>
              </li>
              <li>
              ...
              <li>
                <a href={mailLink()} className="flex items-center gap-2 break-all hover:underline">
                  <Icon name="Mail" className="h-4 w-4 text-brand-800" />
                  {org.email}
                </a>
              </li>
            </ul>
            <p className="mt-4 flex items-start gap-2 text-sm text-ink-soft">
              <Icon name="MapPin" className="mt-0.5 h-4 w-4 shrink-0 text-brand-800" />
        ...
        <div className="border-t border-ink/10">
          <div className="container-x flex flex-col items-center justify-between gap-2 py-5 text-center text-sm text-ink-soft sm:flex-row sm:text-left">
            <p>
...
function FooterCol({ title, links }) {
  return (
    <div>
      <h4 className="text-sm font-bold uppercase tracking-wider text-brand-900">{title}</h4>
      <ul className="mt-4 space-y-2.5 text-sm">
        {links.map((l) => (
          <li key={l.label}>
            <Link to={l.to} className="text-ink-soft transition hover:text-ink hover:underline">
              {l.label}
            </Link>
          </li>
        ))}
